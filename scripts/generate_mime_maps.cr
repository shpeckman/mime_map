...
module MimeMapGen
  ...
  def self.emit_map(io : IO, path : String, constant : String, pairs : Array({String, String})) : Nil
    io << "# " << path << '\n'
    io << "# Generated by gen/mime_map_gen.cr. Do not edit.\n"
    io << '\n'
    io << "module MimeMap\n"
    io << "  " << constant << " = {\n"
  ...
  def self.emit(entries : Array(Entry), output_dir : String) : Nil
    ...
    root_path = File.join(output_dir, "mime_map.cr")
    File.open(root_path, "w") do |io|
      io << "# " << root_path << '\n'
      io << "# Generated by gen/generate_mime_maps.cr. Do not edit.\n"
      io << '\n'
      io << "require \"./mime_map/name_to_type\"\n"
      io << "require \"./mime_map/type_to_name\"\n"
      io << '\n'
      io << "module MimeMap\n"
      io << "  def self.media_type?(name : String) : String?\n"
      io << "    NAME_TO_TYPE[name]?\n"
      io << "  end\n"
      io << '\n'
      io << "  def self.media_type(name : String) : String\n"
      io << "    NAME_TO_TYPE[name]\n"
      io << "  end\n"
      io << '\n'
      io << "  def self.name?(media_type : String) : String?\n"
      io << "    TYPE_TO_NAME[media_type]?\n"
      io << "  end\n"
      io << '\n'
      io << "  def self.name(media_type : String) : String\n"
      io << "    TYPE_TO_NAME[media_type]\n"
...
source     = ARGV[0]? || MimeMapGen::DEFAULT_SOURCE
output_dir = ARGV[1]? || MimeMapGen::DEFAULT_OUTPUT_DIR
MimeMapGen.run(source, output_dir)
